...
| `demo-competitor-agent.html` | 竞品调研 Agent（Tool Calling / 案例表 / OCR） |
| `moments.html` | 生活向 · 朋友圈原型（非主案例） |
| `本地预览.command` | Mac 双击本地预览（免上传） |
...
| `demo-skills-playground.html` | Skills 人设试玩（切换卡片对话，测 ai-pm / web-delivery） |
| `skills-playground/personas.js` | 人设 prompt（与 `~/.cursor/skills` 同步） |
...
## 本地预览（改完刷新即可，不必每次上传 GitHub）

**Mac 最简单：** 在访达里双击项目文件夹里的 **`本地预览.command`**（首次若提示无法打开：右键 → 打开）。会自动打开浏览器并启动本地网站。
...
| 竞品 Agent | http://localhost:8080/demo-competitor-agent.html |
| PRD 项目8 | http://localhost:8080/prd.html?p=8 |

注意：**不要双击 .html 文件**（地址会是 `file://`，GPT 会报错）。用上面两种方式才是 `http://`。

满意后再 [上传 GitHub](https://github.com/ada481131-alt/ada-portfolio/upload) 给外人看即可。
